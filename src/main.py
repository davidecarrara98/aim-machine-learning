...
eval_obj = Evaluator(supported_metrics=['mse', 'mae',
                                        'corr'])  # useremo mean squared error, mean absolute error e correlazione di pearson.
...
print(tuner.tune_parameters({'k': [1, 2, 3, 4, 5]}, eval_type='ttsplit', eval_obj=eval_obj,
                            **{'K': 5, 'test_proportion': 0.2}), file=f)
...
tuner = ParametersTuner(model_class=NeighborRegressor, X=X, y=y, supported_eval_types=['ttsplit', 'kfold'],
                        output_path='output/')
print(tuner.tune_parameters({'k': np.arange(1, 80)}, eval_type='ttsplit', eval_obj=eval_obj, fig_name='ttsplit.png',
                            **{'K': 5, 'test_proportion': 0.2}), file=f)
best_params_kfold = tuner.tune_parameters({'k': np.arange(1, 80)}, eval_type='kfold', eval_obj=eval_obj,
                                          fig_name='kfold.png', **{'K': 5, 'test_proportion': 0.2})
...
try:
    tuner = ParametersTuner(model_class=KNeighborsRegressor, X=X, y=y, supported_eval_types=['kfold'],
                            output_path='output/')
    best_params_skl = tuner.tune_parameters({'n_neighbors': np.arange(1, 80)}, eval_type='kfold', eval_obj=eval_obj,
                                            **{'K': 5, 'test_proportion': 0.2})
...
tuner = ParametersTuner(model_class=MySklearnNeighborRegressor, X=X, y=y, supported_eval_types=['kfold'],
                        output_path='output/')
best_params_skl = tuner.tune_parameters({'n_neighbors': np.arange(1, 80)}, eval_type='kfold', eval_obj=eval_obj,
                                        **{'K': 5, 'test_proportion': 0.2})
...
if data_path in ['data/dataset3.csv', 'data/dataset5.csv']:
    ...
    eval_obj.set_metric('mse')
    tuner = ParametersTuner(model_class=MultipleRegressor, X=X_, y=y, supported_eval_types=['ttsplit', 'kfold'],
                            output_path='output/')
    print(tuner.tune_parameters({'a': np.linspace(0, 5, 11), 'b': np.linspace(0, 5, 11)}, eval_type='ttsplit',
                                eval_obj=eval_obj, **{'K': 5, 'test_proportion': 0.2}), file=f)

    best_params = tuner.tune_parameters({'a': np.linspace(0, 5, 11), 'b': np.linspace(0, 5, 11)}, eval_type='kfold',
                                        eval_obj=eval_obj, **{'K': 5, 'test_proportion': 0.2})
    ...
    tuner = ParametersTuner(model_class=MultipleRegressor, X=X1, y=y, supported_eval_types=['ttsplit', 'kfold'],
                            output_path='output/')
    best_params1 = tuner.tune_parameters({'a': np.linspace(0, 5, 11), 'b': [0]}, eval_type='kfold', eval_obj=eval_obj,
                                         fig_name='kfold_mult_regressor', **{'K': 5, 'test_proportion': 0.2})
    # notiamo come si comporta il parametro a rispetto all'errore (fig. kfold_mult_regressor.png)
    tuner = ParametersTuner(model_class=MultipleRegressor, X=X2, y=y, supported_eval_types=['ttsplit', 'kfold'],
                            output_path='output/')
    best_params2 = tuner.tune_parameters({'a': np.linspace(0, 5, 11), 'b': np.linspace(0, 5, 11)}, eval_type='kfold',
                                         eval_obj=eval_obj, **{'K': 5, 'test_proportion': 0.2})
    ...
    tuner = ParametersTuner(model_class=MultipleRegressor, X=X, y=y, supported_eval_types=['kfold'],
                            output_path='output/')
    best_params = tuner.tune_parameters({'a': [[i, j] for i in np.linspace(-1, 2, 11) for j in np.linspace(-1, 2, 11)],
                                         'b': np.linspace(0, 5, 11)}, eval_type='kfold', eval_obj=eval_obj,
                                        **{'K': 5, 'test_proportion': 0.2})
